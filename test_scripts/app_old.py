...
# Initialize session state for liked songs
...
def _validate_api_key():
    """Validate Gemini API key from sidebar, return key and model."""
    with st.sidebar:
...
def main():
    ...
    with col2:
        target_energy = st.slider(
            "⚡ Target energy level", min_value=0.0, max_value=1.0, value=0.6, step=0.01
        )
    ...
    # Add new song section with input fields
    ...
    if search_button:
        # Build liked songs list from session state
        liked_songs_list = []
        for idx, song_info in enumerate(st.session_state.liked_songs):
        ...
        user = UserProfile(
            ...
            song_list=liked_songs_list,
        )

        st.info(f"🎵 Discovering songs for {user.name}...")

        # Discover songs using Gemini + MusicBrainz
        try:
            service = ArtistDiscoveryService(gemini_api_key=api_key, model=gemini_model)
            ...
            if not songs:
                st.error(
                    "❌ No songs were discovered. Please try different preferences."
                )
                return
            ...
            # Display results
            st.markdown(
                "## 🎵 Your Personalized Recommendations", unsafe_allow_html=True
            )
